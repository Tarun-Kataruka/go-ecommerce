...
import (
	...
	generate "github.com/Tarun-Kataruka/ecommerce/tokens"
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		var cxt, cancel = context.WithTimeout(context.Background(), 10*time.Second)
		defer cancel()
		var user models.User
		...
		err = UserCollection.FindOne(cxt, bson.M{"email": user.Email}).Decode(&founduser)
		defer cancel()
		if err != nil {
...
func ProductViewerAdmin() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		_, err := ProdCollection.InsertOne(ctx, docs)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "could not add the products"})
			return
		}
		defer cancel()
...
func SearchProduct() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.IndentedJSON(http.StatusInternalServerError, "something went wrong while listing the products")
			return
		}
...
func SearchProductByQuery() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		defer cancel()
		cursor, err := ProdCollection.Find(ctx, bson.M{"product_name": bson.M{"$regex": queryParam}})
		if err != nil {
			c.IndentedJSON(404, "something went wrong while searching the products")
			return
		}
